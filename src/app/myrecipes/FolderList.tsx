...
export default function FolderList(props: FolderListProps) {
  ...
  const foldersWithoutAll = folders.filter(folder => folder.name !== allItemsFolder?.name);
  ...
  return (
    <div className="space-y-4 flex flex-col">
      ...
      <Search recipes={folders.find(folder => folder.name === allItemsFolder?.name)?.recipes ?? []} />
