    ...
    <div className="flex-1 lg:max-w-2xl">
          ...
          folderName !== "all"
            ...
            <Dialog>
              <DialogTrigger asChild>
                <Button variant="outline" className="inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 hover:bg-accent hover:text-accent-foreground h-9 py-2 w-9 px-0">
                  <FolderEdit />
                </Button>
              </DialogTrigger>
              <DialogContent className="sm:max-w-[425px]">
                ...
                <div className="grid gap-4 py-4">
                  <div className="grid grid-cols-4 items-center gap-4">
                    ...
                    <Input id="folderName" value="Folder name" className="col-span-3" />
                  </div>
                </div>
                <DialogFooter>
      ...
      </div>
      {
        isEmptyFolder
          ...
          : (
            <RecipeList>
              {folderContents.map(recipe => <RecipeItem key={recipe.id} name={recipe.name} desc={'short description...'} />)}
            </RecipeList>
          )
      }
    </div>
